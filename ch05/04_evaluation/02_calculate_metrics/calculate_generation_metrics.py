...
def main():
    ...
    parser.add_argument(
        "--noise-sensitivity", action=BooleanOptionalAction, default=False
    )
    parser.add_argument(
        "--context-utility-ratio", action=BooleanOptionalAction, default=False
    )
    parser.add_argument(
    ...
    asyncio.run(
        _main(
            ...
            args.loyalty,
            args.hallucination,
            args.noise_sensitivity,
            args.context_utility_ratio,
...
async def _main(
    ...
    if loyalty:
        ...
        with open(output_fn, "w") as f:
            json.dump(
                [kp.dict() for kp in response_loyalty_list],
                f,
                indent=4,
                ensure_ascii=False,
            )
        supported_kp = sum(
            [1 for kp in response_loyalty_list if kp.label == "Relevant"]
        )
    ...
    if hallucination:
        ...
        with open(output_fn, "w") as f:
            json.dump(
                [
                    {"details": [kp.dict() for kp in kp_g], "is_hallucination": l}
    ...
    if noise_sensitivity:
        ...
        for pair_idx, (claim_context, claim_ans) in enumerate(
            response_noise_sensitivity_kp
        ):
        ...
        with open(output_fn, "w") as f:
            json.dump(
                [
                    {"details": [kp.dict() for kp in kp_g], "is_noise": l}
                    for kp_g, l in result_list
                ],
                f,
                indent=4,
        ...
        print(
            f"noise sensitivity score ↓: {noise_sensitivity_score:.3f} ({noise_sensitivity_kp}/{len(result_list)})"
        )
    ...
    if context_utility_ratio:
        ...
        for claim_context, claim_ans in response_context_utility_ratio_kp:
            ...
            if claim_ans.label != "Relevant":
                supported_by_cxt_and_ans = False
            result_list.append(
                ((claim_context, claim_ans), supported_by_cxt, supported_by_cxt_and_ans)
            )

        output_fn = (
            Path(output_dir) / "metrics" / "generation_context_utility_ratio.json"
        )
        Path(output_fn).parent.mkdir(parents=True, exist_ok=True)
        with open(output_fn, "w") as f:
            json.dump(
                [
                    ...
                    for (kp_cxt, kp_ans), l_cxt, l_ans in result_list
                ],
                f,
                indent=4,
                ensure_ascii=False,
        ...
        context_utility_ratio_score = (
            context_utility_ratio_num / context_utility_ratio_den
            if context_utility_ratio_den
            else 0
        )
        print(
            f"context utility ratio score ↑: {context_utility_ratio_score:.3f} ({context_utility_ratio_num}/{context_utility_ratio_den})"
        )
