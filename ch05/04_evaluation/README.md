...
│   ├── calculate_global_metrics.py         # 全局精确率、召回率
│   ├── calculate_retrieval_metrics.py      # 检索模块指标
│   ├── calculate_generation_metrics.py     # 生成模块指标
...
└── 03_generate_qa_pairs/        # 合成问答对生成
    ├── generate_qa_pairs.py     # 生成问答对
    ├── validate_qa_pairs.py     # 校验问答对
    ├── rewrite_qa_pairs.py      # 重写问答对
    └── prompts/
...
python 03_generate_qa_pairs/generate_qa_pairs.py --index_dir data_chroma --collection_name olympic_games --output_dir data_eval --num_docs 2
```

读取QA对，使用大模型进行验证，将验证结果保存在`data_eval/qa_pairs.validated.json`

```bash
python 03_generate_qa_pairs/validate_qa_pairs.py --input_fn data_eval/qa_pairs.raw.json --output_dir data_eval
```

读取QA对，过滤掉不符合要求的部分。对于剩余的问答对，使用大模型进行重写，将重写的结果保存在`data_eval/qa_pairs.rewritten.json`

```bash
python 03_generate_qa_pairs/rewrite_qa_pairs.py --input_fn data_eval/qa_pairs.validated.json --output_dir data_eval
```
...
```bash
python 01_extract_keypoints/extract_keypoints.py --ground-truth --response --output_dir naive_rag/ --input_fn naive_rag/response.json
```
...
```bash
cd ch05/04_evaluation
python 02_calculate_metrics/calculate_global_metrics.py --num_docs 6 --precision --recall --output_dir naive_rag --input_fn naive_rag/keypoints.json
```

使用`naive_rag/keypoints.json`作为输入，计算检索模块的上下文精度和关键点召回率

```bash
python 02_calculate_metrics/calculate_retrieval_metrics.py --num_docs 2 --precision --recall --output_dir naive_rag --input_fn naive_rag/keypoints.json
```

使用`naive_rag/keypoints.json`作为输入，计算生成模块的忠诚度、幻觉度、噪声敏感度和上下文利用率

```bash
python 02_calculate_metrics/calculate_generation_metrics.py --num_docs 2 --loyalty --hallucination --noise-sensitivity --context-utility-ratio --output_dir naive_rag --input_fn naive_rag/keypoints.json
```
...
| 标题 | 描述 | 文件 |
| --- | --- | --- |
| 5.4.1 细颗粒度关键信息 | 从问答对中提取关键点用于评估 | [extract_keypoints.py](01_extract_keypoints/extract_keypoints.py) |
| 5.4.2 计算全局指标 | 计算全局的精确率和召回率 | [calculate_global_metrics.py](02_calculate_metrics/calculate_global_metrics.py) |
| 5.4.2 计算检索指标 | 计算检索模块的精确率和召回率 | [calculate_retrieval_metrics.py](02_calculate_metrics/calculate_retrieval_metrics.py) |
| 5.4.2 计算生成指标 | 计算生成模块的忠实度、幻觉率、噪声敏感度等指标 | [calculate_generation_metrics.py](02_calculate_metrics/calculate_generation_metrics.py) |
| 5.4.3 生成合成问答对 | 基于文档内容自动生成问答对 | [generate_qa_pairs.py](03_generate_qa_pairs/generate_qa_pairs.py) |
| 5.4.3 校验问答对 | 对生成的问答对进行质量验证 | [validate_qa_pairs.py](03_generate_qa_pairs/validate_qa_pairs.py) |
| 5.4.3 重写问答对 | 对验证通过的问答对进行改写优化 | [rewrite_qa_pairs.py](03_generate_qa_pairs/rewrite_qa_pairs.py) |
