...
# --- Usage Function ---
usage() {
    echo "Usage: $0 --input-fn <path> --num-docs <int>"
    echo ""
    echo "Options:"
    echo "  --input-fn <path>  : Path to the response JSON file generated by generate_synthetic_qa_pairs.sh (e.g., data_eval/naive_rag/response.json)"
    ...
    echo ""
    echo "Example:"
    echo "  $0 --input-fn ./eval_data_20231231/response.json --num-docs 150"
}
...
# --- Validate Arguments ---
if [ -z "$INPUT_FN" ] || [ -z "$RAG_OUTPUT_DIR" ] || [ -z "$NUM_METRIC_DOCS" ]; then
...
if [ ! -f "$INPUT_FN" ]; then
    echo "Error: Input file '$INPUT_FN' not found."
    exit 1
fi
...
python 01_extract_keypoints.py \
    ...
    --input_fn "$INPUT_FN" || { echo "Error during RAG keypoint extraction step. Aborting."; exit 1; }
