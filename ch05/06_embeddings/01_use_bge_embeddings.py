"""BGE Embeddings RAG - 统一的索引与检索问答脚本
...
使用示例:
    # 仅构建索引
    python 01_use_bge_embeddings.py --index --index_dir data_chroma --collection_name olympic_games \
        --index_input_dir ../data

    # 仅查询
    python 01_use_bge_embeddings.py --query --index_dir data_chroma --collection_name olympic_games \
        --query_input_path ../05_chunking/data_eval/keypoints.json --output_dir data_eval/

    # 同时执行索引和查询
    python 01_use_bge_embeddings.py --index --query --index_dir data_chroma --collection_name olympic_games \
        --index_input_dir ../data \
        --query_input_path ../05_chunking/data_eval/keypoints.json --output_dir data_eval/
"""
